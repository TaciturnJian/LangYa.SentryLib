...
#include <LangYa/Network/ServerInfo.hpp>
#include <LangYa/Common/Counter.hpp>

namespace LangYa::Network
{
	/// @brief 一种通用的服务器实现，理论可以接收任意数量客户端。
	///	@details 需要提供一个服务信息，它包含接收器、处理器、中断信号，详见 ServerInfo 模板。
	...
	public:
		...
		void Run()
		{
			...
			try
			{
				...
				Common::Counter acceptor_failure_counter{Info.MaxAcceptorFailureCount};
